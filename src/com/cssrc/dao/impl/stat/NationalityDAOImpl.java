...
public class NationalityDAOImpl extends HibernateDaoSupport implements INationalityDAO {
	...
	public List getSysnationalitystat() {
		...
		try {
			...
			// TODO 常委、委员子库;
			ResultSet rs = stmt.executeQuery("select a.Assoname, b.number, b.china, b.chinaratio, b.foreignnationality, b.foreignnationalityratio from t_shassociationinfo a left join (select AssoName,count(*) as number, sum(case when MemNationality_S='中国籍' then 1 else 0 end) as 'china',sum(case when MemNationality_S='中国籍' then 1 else 0 end)/count(*) as 'chinaratio',sum(case when MemNationality_S='外国籍' then 1 else 0 end) as 'foreignnationality',sum(case when MemNationality_S='外国籍' then 1 else 0 end)/count(*) as 'foreignnationalityratio' from v_person_properties where assoname not in ('常委、委员子库') group by AssoName) b on a.Assoname = b.Assoname where a.Assoname not in ('常委、委员子库')");
			while (rs.next()) {
	...
	public List getAcadnationalitystat(Integer shai_id) {
		...
		try {
			conn = s.connection();
			Statement stmt = conn.createStatement();
			String sql = "select a.acadname, b.number, b.china, b.chinaratio, b.foreignnationality, b.foreignnationalityratio from (select Acadname from t_academybaseinfo where shai_id=" + shai_id + ") a left join (select acadname,count(*) as number, sum(case when MemNationality_S='中国籍' then 1 else 0 end) as 'china',sum(case when MemNationality_S='中国籍' then 1 else 0 end)/count(*) as 'chinaratio',sum(case when MemNationality_S='外国籍' then 1 else 0 end) as 'foreignnationality',sum(case when MemNationality_S='外国籍' then 1 else 0 end)/count(*) as 'foreignnationalityratio' from v_person_properties c where c.assoname not in ('常委、委员子库') and c.shai_id = " + shai_id + " group by acadname) b on a.AcadName = b.AcadName";
	...
	public List getAcademynationalitystat(Integer abi_id) {
		...
		try {
			conn = s.connection();
			Statement stmt = conn.createStatement();
			String sql = "select acadno, acadname,count(*) as number, sum(case when MemNationality_S='中国籍' then 1 else 0 end) as 'china',sum(case when MemNationality_S='中国籍' then 1 else 0 end)/count(*) as 'chinaratio',sum(case when MemNationality_S='外国籍' then 1 else 0 end) as 'foreignnationality',sum(case when MemNationality_S='外国籍' then 1 else 0 end)/count(*) as 'foreignnationalityratio' from v_person_properties a where a.assoname not in ('常委、委员子库') and a.abi_id = " + abi_id + " group by abi_id";
			ResultSet rs = stmt.executeQuery(sql);
			while (rs.next()) {
				TempBean bean = new TempBean();
